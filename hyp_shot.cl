...
int hyp_shot(RESULT_TYPE *res, const double3 r_pt, const double3 r_dir, const uint idx, global const struct hyp_specific *hyp)
{
    ...
    dp.x = dot(hyp->hyp_Aunit, r_dir);
    dp.y = dot(hyp->hyp_Bunit, r_dir);
    dp.z = dot(hyp->hyp_Hunit, r_dir);

    xlated = r_pt - vload3(0, hyp->hyp_V);
    pp.x = dot(hyp->hyp_Aunit, xlated);
    pp.y = dot(hyp->hyp_Bunit, xlated);
    pp.z = dot(hyp->hyp_Hunit, xlated);
